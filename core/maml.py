...
class MAML:
    ...
    def _inner_loop(self, images, labels, train):   # pylint: disable=unused-argument
        ...
        for i in range(self._num_inner_steps):
            logits = self._forward(images=images, parameters=parameters)
            inner_loss = F.cross_entropy(logits, labels)
            
            accuracies.append(util.score(logits=logits, labels=labels))
            grads = autograd.grad(inner_loss, parameters.values(), create_graph=train)
            
            for (j, n) in enumerate(parameters.keys()):
                parameters[n] = parameters[n] - self._inner_lrs[n] * grads[j]
